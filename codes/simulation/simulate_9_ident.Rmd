...
syn_lib_together=read.csv("syn_lib_9_metabolites.csv",sep=",")
rownames(syn_lib_together)=syn_lib_together[,1]
syn_lib_together=syn_lib_together[,-1]
dim(syn_lib_together)
#    9 40000
...
sim_fun=function(n,shape,rate,ppm_step,syn_lib_together,real_lib_together, d, output_dir){
  num=sample(1:9,1)
  ...
  syn_compound=compound[-real_index]
  
  syn_spec=syn_lib_together[syn_compound,]
...
##leave the first concentration for the real metabolite
  ...
  syn_spectra=colSums(c(concentration[-1])*syn_spec)
  ...
  total_spectra=colSums(rbind(concentration[1]*real_lib_together[real_compound,],syn_spectra))
...
mclapply(1:10000, FUN=sim_fun,
         ...
         ppm_step=ppm_step,
         syn_lib_together=syn_lib_together,
